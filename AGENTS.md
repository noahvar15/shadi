...
- All dashboard API calls must use the `/api/` prefix (e.g. `/api/cases`, `/api/reports/${id}`); the axios client uses same-origin by default so requests flow through Next.js rewrites to FastAPI. `NEXT_PUBLIC_API_URL` must **never** be set at runtime — it exists only in `vitest.config.ts` for MSW test interception.
- `dashboard/next.config.ts` adds a rewrite `/api/:path* → ${API_URL}/:path*` (server-only env var, default `http://localhost:8000`). Do NOT use `NEXT_PUBLIC_API_URL` for the rewrite — that leaks to the browser bundle and makes axios bypass the rewrite.
